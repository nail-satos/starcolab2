...
def sakusei(char_count=-1):
    ...
    for i in range(max_count+1):
        ...
        # 画像の重ね合わせ
        rndx = random.randint(-20, 780)  # x座標のmaxは800
        rndy = random.randint(-20, 520)  # y座標のmaxは540
    ...
    # 生成した画像をセッションステートに保存
    st.session_state['img_exec'] = img_pil_base
...
def sagase():
    ...
    img_pil_base = st.session_state['img_exec']
...
#             # 画像の重ね合わせ
#             rndx = random.randint(10, 780)  # x座標のmaxは800
#             rndy = random.randint(10, 520)  # y座標のmaxは540
#             img_pil_base.paste(img_pil_layer, (rndx, rndy), img_pil_layer)
